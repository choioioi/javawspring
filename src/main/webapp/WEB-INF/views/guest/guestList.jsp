  ...
  <c:forEach var="vo" items="${vos}" varStatus="st">
	  ...
	  <table class="table table-bordered">
			<tr>
			  ...
			  <td style="width:35%;">${fn:substring(vo.visitDate,0,19)}</td> <!-- 19번 앞에까지 잘라주세요 -->
			</tr>
			<tr>
			  <th>전자우편</th>
			  <td colspan="3">
			...
			<tr>
			  <th>홈페이지</th>
			  <td colspan="3">
			    <c:if test="${fn:length(vo.homePage) <= 8}">- 없음 -</c:if> <!-- 최소 8보다 작거나 같으면 홈페이지가 없는 것 -->
  ...
  <div class="text-center">
  	<ul class="pagination justify-content-center">
	    ...
	    <c:forEach var="i" begin="${(pageVo.curBlock)*pageVo.blockSize + 1}" end="${(pageVo.curBlock)*pageVo.blockSize + pageVo.blockSize}" varStatus="st">
	    	<c:if test="${i <= pageVo.totPage && i == pag}"> <!-- i가 토탈페이지보다 작거나 같으면 끝내라, 마지막이 5페이지인데 456에서 6페이지까지 나오면 안되니까 -->
